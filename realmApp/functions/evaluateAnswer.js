exports = async function (arg) {
    ...
    const matchingQuestion = quiz?.questions?.find((element) => element._id === questionId);
    if (!matchingQuestion) {
    ...
    let totalCorrect = isCorrect === true ? 1 : 0, numAnswered = 1;
    ...
    async function createUserEvent(event, data) {
        if (!event)
            return;
        const newEvent = {
            userId,
